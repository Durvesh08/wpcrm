...
(function() {
  ...
  greeting.style.padding = '14px 18px';
  greeting.style.borderRadius = '12px';
  greeting.style.boxShadow = '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)';
  greeting.style.marginBottom = '16px';
  greeting.style.fontSize = '14px';
  greeting.style.lineHeight = '1.4';
  greeting.style.fontWeight = '500';
  greeting.style.maxWidth = '250px';
  greeting.style.opacity = '0';
  greeting.style.transform = 'translateY(10px) scale(0.95)';
  greeting.style.transition = 'all 0.3s cubic-bezier(0.16, 1, 0.3, 1)';
  ...
  ${position === 'bottom-left' ? 'tail.style.left = "24px";' : 'tail.style.right = "24px";'}
  tail.style.width = '12px';
  tail.style.height = '12px';
  tail.style.backgroundColor = '#ffffff';
  tail.style.transform = 'rotate(45deg)';
  tail.style.boxShadow = '5px 5px 5px -3px rgba(0, 0, 0, 0.05)';
  
  const textContent = document.createElement('div');
  textContent.innerText = ${JSON.stringify(greeting_text || '')};
  
  greeting.appendChild(textContent);
  greeting.appendChild(tail);
  ...
  button.style.boxShadow = '0 10px 25px -5px rgba(37, 211, 102, 0.4)';
  button.style.cursor = 'pointer';
  button.style.transition = 'transform 0.25s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.25s ease';
  
  // WhatsApp Icon SVG
  button.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="white" viewBox="0 0 24 24"><path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.77-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.299.045-.677.063-1.092-.069-.252-.08-.575-.187-.988-.365-1.739-.751-2.874-2.502-2.961-2.617-.087-.116-.708-.94-.708-1.793s.448-1.273.607-1.446c.159-.173.346-.217.462-.217l.332.006c.106.005.249-.04.39.298.144.347.491 1.2.534 1.287.043.087.072.188.014.304-.058.116-.087.188-.173.289l-.26.304c-.087.086-.177.18-.076.354.101.174.449.741.964 1.201.662.591 1.221.774 1.394.86s.274.072.383-.043c.108-.116.477-.547.606-.702.13-.153.255-.118.41-.043s.983.462 1.151.546c.168.084.281.127.323.196.043.069.043.4-.101.805z"/></svg>';
  
  button.onmouseover = function() { 
    button.style.transform = 'scale(1.08)';
    button.style.boxShadow = '0 15px 30px -5px rgba(37, 211, 102, 0.5)';
