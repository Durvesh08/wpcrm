...
import { repairAccountContext } from './repair-account-context';
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  if (!data || !data.account_id || !data.account_role) {
    ...
    // signup trigger. Repair once with the service-role client before
    // giving up so old users can log in after the account-sharing
    // migration.
    try {
      await repairAccountContext(user);
      const repaired = await loadProfile();
      data = repaired.data;
      error = repaired.error;
    } catch (repairErr) {
      console.error('[getCurrentAccount] profile repair failed:', repairErr);
    }

    if (error) {
      console.error('[getCurrentAccount] profile refetch error:', error);
      throw new ForbiddenError('Could not load account context');
    }
  ...
  if (!account) {
    ...
    // or an RLS gap. Try the same repair path once, then reload both
    // profile and account because the account_id may change.
    try {
      await repairAccountContext(user);
      const repaired = await loadProfile();
      data = repaired.data;
      ...
      if (!error && data?.account_id) {
        const accountReload = await supabase
          .from('accounts')
          .select('id, name')
          .eq('id', data.account_id)
          .maybeSingle();
        account = accountReload.data;
        accountErr = accountReload.error;
      }
    } catch (repairErr) {
      console.error(
        '[getCurrentAccount] orphaned account repair failed:',
        repairErr
      );
    }
    ...
    if (accountErr) {
      console.error('[getCurrentAccount] account refetch error:', accountErr);
      throw new ForbiddenError('Could not load account context');
    }
    if (!data || !data.account_id || !data.account_role || !account) {
      throw new ForbiddenError('Profile is not linked to an account');
    }
  }
