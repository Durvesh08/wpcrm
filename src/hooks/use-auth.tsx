...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const repairProfile = useCallback(async () => {
    try {
      const response = await fetch('/api/auth/repair-profile', {
        method: 'POST',
        cache: 'no-store',
      });

      if (!response.ok) {
        console.error('[AuthProvider] repairProfile failed:', response.status);
      }
    } catch (err) {
      console.error('[AuthProvider] repairProfile threw:', err);
    }
  }, []);
    ...
    const init = async () => {
      try {
        ...
        if (currentUser) {
          ...
          void repairProfile().finally(() => fetchProfile(currentUser.id));
        } else {
