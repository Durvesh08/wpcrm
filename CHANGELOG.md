...
Assistant**; ZOVAIX calls the provider directly with that key, so
...
- **Configurable default deal currency.** Each account can now pick
  ...
  [Default currency](https://wpcrm-wine.vercel.app/docs/settings#deals).
- **Members tab in Settings.** The user-facing surface for the
  ...
  [Members docs](https://wpcrm-wine.vercel.app/docs/members).
