-- 036_managed_ai_credits.sql — included Gemini allowance for ZOVAIX plans

ALTER TABLE public.ai_configs
  ADD COLUMN IF NOT EXISTS platform_ai_enabled boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  -- Browser callers may only spend their own Copilot allowance. The
  -- WhatsApp webhook uses the service role and is allowed to debit the
  -- account owner's automated-reply allowance.
  IF auth.role() <> 'service_role' AND target_user_id <> auth.uid() THEN
    RAISE EXCEPTION 'Cannot claim credits for another user';
  END IF;

  -- Do not trust a caller-supplied limit: otherwise an authenticated
  -- client could request an arbitrary allowance through this RPC.
